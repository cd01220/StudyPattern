...
// Listing 3 code/ch12
int Message_Receiver::copy_payload (ACE_Message_Block *mb,
                                    int payload_length)
{
    ssize_t result =
...
static int ClientMain(u_short port)
{
    ...
    char* str = "abc";
    DeviceCommandHeader dch = {strlen(str) + 1, 1};

    int bc;
    char buf[64];
    memcpy(buf, &dch, sizeof(dch));
    strcpy(buf + sizeof(DeviceCommandHeader), str);

    peer.send_n (buf, sizeof(DeviceCommandHeader) + strlen(str) + 1);
...
int AceMessageQueueDemoMain(int argc, ACE_TCHAR *argv[])
{
    u_short port = 5000;
    ...
    std::thread thd(ClientMain, port);
